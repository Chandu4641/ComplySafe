...
function run() {
  ...
  const verification = readJson("docs/verification/phase5-verification.json");
  const matrix = readJson("docs/verification/phase5-regression-matrix.json");
  const regression = readJson("docs/verification/phase5-regression-report.json");
  const workflow = readText(".github/workflows/phase5-release-gate.yml");
  ...
  results.push(
    buildResult(
      ...
      verifyPass,
      verifyPass
        ? `phase5-verification.json: ${verification.passed}/${verification.total} checks passed`
        : "phase5-verification.json missing or not fully passing",
      verifyPass ? [] : ["Run npm run phase5:verify and commit updated report"]
    )
  );
  ...
  results.push(
    buildResult(
      ...
      hasRegressionMatrix ? [] : ["Define at least 10 regression checks in docs/verification/phase5-regression-matrix.json"]
    )
  );
  ...
  results.push(
    buildResult(
      ...
      regressionPass,
      regressionPass
        ? `phase5-regression-report.json: ${regression.passed}/${regression.total} checks passed`
        : "phase5-regression-report.json missing or not fully passing",
      regressionPass ? [] : ["Run npm run phase5:regression and commit updated report"]
    )
  );
  ...
  const ciTokens = [
    ...
    "npm run phase5:regression",
    "npm run phase5:closure -- --strict",
  ...
  results.push(
    buildResult(
      ...
      ciReady,
      ciReady
        ? "Workflow executes Phase 5 gate and publishes evidence artifacts"
        : "Workflow is missing required gate steps or artifact publishing",
      ciReady ? [] : ["Update .github/workflows/phase5-release-gate.yml with full release gate steps"]
    )
  );
  ...
  const outPath = path.join(root, "docs", "verification", "phase5-closure-status.json");
