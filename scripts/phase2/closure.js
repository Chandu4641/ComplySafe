...
function run() {
  ...
  const verification = readJson("docs/verification/phase2-verification.json");
  const matrix = readJson("docs/verification/phase2-regression-matrix.json");
  const regression = readJson("docs/verification/phase2-regression-report.json");
  const workflow = readText(".github/workflows/phase2-release-gate.yml");
  ...
  results.push(
    buildResult(
      ...
      verifyPass,
      verifyPass
        ? `phase2-verification.json: ${verification.passed}/${verification.total} checks passed`
        : "phase2-verification.json missing or not fully passing",
      verifyPass ? [] : ["Run npm run phase2:verify and commit updated report"]
    )
  );
  ...
  results.push(
    buildResult(
      ...
      hasRegressionMatrix ? [] : ["Define at least 10 regression checks in docs/verification/phase2-regression-matrix.json"]
    )
  );
  ...
  results.push(
    buildResult(
      ...
      regressionPass,
      regressionPass
        ? `phase2-regression-report.json: ${regression.passed}/${regression.total} checks passed`
        : "phase2-regression-report.json missing or not fully passing",
      regressionPass ? [] : ["Run npm run phase2:regression and commit updated report"]
    )
  );
  ...
  results.push(
    buildResult(
      ...
      ciReady,
      ciReady
        ? "Workflow executes Phase 2 gate and publishes evidence artifacts"
        : "Workflow is missing required gate steps or artifact publishing",
      ciReady ? [] : ["Update .github/workflows/phase2-release-gate.yml with full release gate steps"]
    )
  );
  ...
  const outPath = path.join(root, "docs", "verification", "phase2-closure-status.json");
  fs.writeFileSync(outPath, `${JSON.stringify(report, null, 2)}\n`);
  const mirrorPath = path.join(root, "docs", "phase2-closure-status.json");
  const mirror = {
    phase: "Phase 2 — Multi-Framework Expansion",
    status,
    updatedAt: report.generatedAt,
    sourceOfTruth: "docs/verification/phase2-closure-status.json",
    summary: {
