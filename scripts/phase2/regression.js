...
function runCheck(check) {
  ...
  if (check.type === "jsonFieldsEqual") {
    ...
    for (const [field, expected] of Object.entries(check.fields || {})) {
      ...
      if (actual !== expected) mismatches.push(`${field} expected=${JSON.stringify(expected)} actual=${JSON.stringify(actual)}`);
    }
    return {
      pass: mismatches.length === 0,
      detail: mismatches.length === 0 ? `JSON fields match in ${check.file}` : `JSON mismatch in ${check.file}: ${mismatches.join("; ")}`
    };
  }
...
function run() {
  const matrixPath = "docs/verification/phase2-regression-matrix.json";
  ...
  const outputPath = resolve("docs/verification/phase2-regression-report.json");
